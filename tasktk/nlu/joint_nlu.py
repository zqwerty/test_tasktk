...
import copy, pickle, argparse, json
import numpy as np
...
class joint_nlu:
    ...
    def parse(self, string):
        """ parse string into dialog act """
        
        dia_act = self.generate_dia_act(string)
        return_dia_act = {dia_act['diaact']:[]}
        for slot in dia_act['inform_slots']:
            return_dia_act[dia_act['diaact']].append((slot, dia_act['inform_slots'][slot]))
        
        for slot in dia_act['request_slots']:
            return_dia_act[dia_act['diaact']].append((slot, '?'))
            
        return return_dia_act
...
def convert_pickle(params):
    ...
    model_params = pickle.load(open(nlu_model_path, 'rb'), encoding='latin1')
...
def test_nlu(params):
    ...
    nl = "yes. it should be moderately priced."
    dia_act = nlu_model.generate_dia_act(nl)
    print('test case 1:')
    print('nl: %s' % (nl))
    print('dia_act: %s' % (json.dumps(dia_act, indent=2)))
    
    
    nl = "i want to book a table for 6 at 18:45 on thursday"
    dia_act = nlu_model.generate_dia_act(nl)
    print('test case 2:')
    print('nl: %s' % (nl))
    print('dia_act: %s' % (json.dumps(dia_act, indent=2)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nlu_model_path', dest='nlu_model_path', type=str, default='../../data/models/nlu/bi_lstm_[1549553112.36]_27_29_400_0.882.p', help='path to nlu model file')
