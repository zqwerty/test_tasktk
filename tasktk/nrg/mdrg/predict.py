...
from tasktk.nrg.mdrg.utils import util
from tasktk.nrg.mdrg.model.model import Model
from tasktk.nrg.mdrg.utils.nlp import normalize
# from tasktk.nrg.mdrg.utils import dbPointer
from tasktk.nrg.mdrg.utils import delexicalize
from tasktk.nrg.mdrg.utils import dbquery 


DATA_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'data/nrg/mdrg')
...
def addDBPointer(state):
    ...
    for domain in domains:
        ...
        pointer_vector = dbquery.oneHotVector(len(entities), domain, pointer_vector)
...
def predict(model, prev_state, state, dic):
    ...
    output_words, loss_sentence = model.predict(input_tensor, input_lengths, input_tensor, input_lengths,
                                                db_tensor, bs_tensor)
    active_domain = get_active_domain(prev_state, state)
...
def get_active_domain(prev_state, state):
    domains = ['taxi', 'restaurant', 'hospital', 'hotel', 'attraction', 'train', 'police']
    print('double check')
    for domain in domains:
        print(domain)
